...
| `dsp/pipeline/stages.py` | Keep, split later | Real DSP stage orchestration. Good future split: phase metrics and DoA metrics. |
| `gnss/constellations.py` | Keep | Single place for GPS, BeiDou, and GLONASS filtering/display labels. |
...
| `runtime/ui_metrics.py` | Keep | UI payload model. Consider grouping fields by phase/DoA/GNSS if callers get cleaner. |
...
| `ui/widgets/plots/algorithm.py` | Keep | Shared pyqtgraph styling and DoA/PRN plot builders. |
