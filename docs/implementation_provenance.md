...
| RF link budget, attenuation, and geometry | Derived, measured, or assumed per field | Product code must not treat bladeRF distance/gain, jammer distance/attenuation, or bench geometry as intrinsic realtime controls. Such values live in experiment overlays/audits and need field-level labels for measured versus assumed inputs. | `01_rf_hardware_and_link_budget.md`, `hardware.md`, and retained audits |
