...
| Ideal and measured-vector constraints | Project-specific application | The preserve/null vectors, all-ones default preserve response, measured dominant-U1 option, loading values, conditioning limit, and maximum norm are repository decisions. A mathematically deep model null is not an OTA suppression measurement. | Code, runtime profile, and retained live records |
...
| RF link budget, attenuation, and geometry | Derived, measured, or assumed per field | Product code does not treat bladeRF distance/gain, jammer distance/attenuation, expected bearing, or bench geometry as realtime controls. The calculator is an offline diagnostic used with explicit historical/operator inputs; values live in retained audits and need field-level labels for measured versus assumed inputs. | `01_rf_hardware_and_link_budget.md`, `hardware.md`, and retained audits |
