...
void pcps_acquisition::log_acquisition(const AcquisitionResult& result) const
{
    ...
    LOG(INFO) << "Acquisition decision: "
              << (result.positive_acq ? "positive" : "negative")
              << ", satellite " << d_gnss_synchro->System << " " << d_gnss_synchro->PRN
              << ", sample_stamp " << result.sample_count
              << ", test_statistics " << result.test_statistics
              << ", threshold " << threshold
              << ", threshold_margin " << (result.test_statistics - threshold)
              << ", threshold_ratio " << (threshold != 0.0F ? result.test_statistics / threshold : 0.0F)
              << ", code_phase " << d_gnss_synchro->Acq_delay_samples
              << ", doppler " << static_cast<double>(result.doppler)
              << ", input_signal_power " << d_input_power
              << ", assist_doppler_center " << d_doppler_center;
}
