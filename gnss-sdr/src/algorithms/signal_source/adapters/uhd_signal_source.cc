    ...
    for (int i = 0; i < RF_channels_; i++)
        {
            ...
            if (!antenna_.empty())
                {
                    uhd_source_->set_antenna(antenna_, i);
                    std::cout << "RX antenna set to: " << uhd_source_->get_antenna(i) << "\n";
                    LOG(INFO) << "RX antenna set to: " << uhd_source_->get_antenna(i);
                }
                    ...
                    for (int attempt = 0; attempt < 30; attempt++)
                        {
                            ...
                            locked = lo_locked.to_bool();
                            if (locked)
                                {
                                    std::cout << "Check for front-end " << lo_locked.to_pp_string() << " is ... Locked\n";
                                    LOG(INFO) << "Front-end LO locked on channel " << i;
                                    break;
                                }
                            if (attempt == 0)
                                {
                                    std::cout << "Check for front-end " << lo_locked.to_pp_string() << " is ... waiting\n";
                                    LOG(INFO) << "Front-end LO not yet locked on channel " << i << ", waiting...";
                                }
                            std::this_thread::sleep_for(std::chrono::milliseconds(100));
                        }
                    if (!locked)
                        {
                            std::cout << "Check for front-end lo_locked is ... UNLOCKED after wait\n";
                            LOG(WARNING) << "Front-end LO still unlocked after wait on channel " << i;
                        }
                }
        }
