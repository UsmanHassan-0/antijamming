...
{
    auto *out = static_cast<gr_complex *>(output_items.at(0));
    const auto requested_bytes = static_cast<std::streamsize>(noutput_items * sizeof(gr_complex));
    fifo_.read(reinterpret_cast<char *>(out), requested_bytes);

    const auto bytes_read = fifo_.gcount();
    const auto items_retrieved = static_cast<size_t>(bytes_read / sizeof(gr_complex));
