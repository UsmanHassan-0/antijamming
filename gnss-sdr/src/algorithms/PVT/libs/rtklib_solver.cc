...
void Rtklib_Solver::log_rtklib_residuals()
{
    if (!d_conf.log_rtklib_residuals)
        {
            return;
        }

    int week = 0;
    const double tow_s = time2gpst(pvt_sol.time, &week);
    const double period_s = static_cast<double>(std::max<int32_t>(100, d_conf.rtklib_residual_log_period_ms)) / 1000.0;
    if (d_last_residual_log_tow_s >= 0.0)
        {
            double elapsed_s = tow_s - d_last_residual_log_tow_s;
            if (elapsed_s < 0.0)
                {
                    elapsed_s += 604800.0;
                }
    ...
    d_last_residual_log_tow_s = tow_s;

    constexpr double rad_to_deg = 57.2957795130823208768;
    constexpr double active_eps = 1e-12;
    int active_count = 0;
    int used_single_count = 0;
    int used_l1_count = 0;
    int rejected_l1_count = 0;
    int slip_l1_count = 0;
    int resp_count = 0;
    double resp_sum_m = 0.0;
    double resp_sq_sum_m2 = 0.0;
    double worst_abs_resp_m = -1.0;
    double worst_resp_m = 0.0;
    std::string worst_sat;
    ...
    for (int sat = 1; sat <= MAXSAT; ++sat)
        {
            ...
            const ssat_t& sat_status = d_rtk.ssat[sat - 1];
            const double resp_m = sat_status.resp[0];
            const double resc_m = sat_status.resc[0];
            const bool active =
                sat_status.vs != 0 ||
                sat_status.vsat[0] != 0 ||
                sat_status.snr[0] != 0 ||
                sat_status.slip[0] != 0 ||
                sat_status.rejc[0] != 0 ||
                std::fabs(resp_m) > active_eps ||
                std::fabs(resc_m) > active_eps ||
                std::fabs(sat_status.azel[0]) > active_eps ||
                std::fabs(sat_status.azel[1]) > active_eps;
            if (!active)
                {
                    continue;
                }
            ...
            if (sat_status.vs != 0)
                {
                    used_single_count++;
                    if (std::isfinite(resp_m))
                        {
                            resp_count++;
                            resp_sum_m += resp_m;
                            resp_sq_sum_m2 += resp_m * resp_m;
                            const double abs_resp_m = std::fabs(resp_m);
                            if (abs_resp_m > worst_abs_resp_m)
                                {
                                    worst_abs_resp_m = abs_resp_m;
                                    worst_resp_m = resp_m;
                                    worst_sat = satno2id(sat);
                                }
                        }
                }
            if (sat_status.vsat[0] != 0)
                {
                    used_l1_count++;
                }
            if (sat_status.rejc[0] != 0)
                {
                    rejected_l1_count++;
                }
            if (sat_status.slip[0] != 0)
                {
                    slip_l1_count++;
                }
            ...
            line << std::fixed << std::setprecision(3)
                 << "RTKLIB_PVT_RESIDUAL tow_s=" << tow_s
                 << " week=" << week
                 << " sat=" << satno2id(sat)
                 << " rtklib_sat=" << sat
                 << " sys_mask=" << sys
                 << " prn=" << prn
                 << " used_single=" << static_cast<int>(sat_status.vs)
                 << " vsat_l1=" << static_cast<int>(sat_status.vsat[0])
                 << " az_deg=" << sat_status.azel[0] * rad_to_deg
                 << " el_deg=" << sat_status.azel[1] * rad_to_deg
                 << " resp_m=" << resp_m
                 << " resc_m=" << resc_m
                 << " snr_dbhz=" << static_cast<double>(sat_status.snr[0]) * 0.25
                 << " slip_l1=" << static_cast<int>(sat_status.slip[0])
                 << " rejc_l1=" << sat_status.rejc[0]
                 << " lock_l1=" << sat_status.lock[0]
                 << " outc_l1=" << sat_status.outc[0];
            residual_lines.push_back(line.str());
        }

    const double resp_mean_m = resp_count > 0 ? resp_sum_m / static_cast<double>(resp_count) : std::numeric_limits<double>::quiet_NaN();
    const double resp_rms_m = resp_count > 0 ? std::sqrt(resp_sq_sum_m2 / static_cast<double>(resp_count)) : std::numeric_limits<double>::quiet_NaN();
    std::ostringstream summary;
    summary << std::fixed << std::setprecision(3)
            << "RTKLIB_PVT_RESIDUAL_SUMMARY tow_s=" << tow_s
            << " week=" << week
            << " sol_status=" << static_cast<int>(pvt_sol.stat)
            << " sol_ns=" << static_cast<int>(pvt_sol.ns)
            << " active_sats=" << active_count
            << " used_single=" << used_single_count
            << " used_l1=" << used_l1_count
            << " rejected_l1=" << rejected_l1_count
            << " slip_l1=" << slip_l1_count
            << " resp_count=" << resp_count
            << " resp_mean_m=" << resp_mean_m
            << " resp_rms_m=" << resp_rms_m
            << " worst_sat=" << (worst_sat.empty() ? "none" : worst_sat)
            << " worst_resp_m=" << worst_resp_m
            << " gdop=" << d_dop[0]
            << " pdop=" << d_dop[1]
            << " hdop=" << d_dop[2]
            << " vdop=" << d_dop[3];
