...
class OutputMonitorMixin:
    def _prepare_nmea_tty(self) -> None:
        ...
        try:
            ...
            self._close_nmea_tty_fds()
            self._nmea_tty_path = "/dev/null"
            raise RuntimeError("Failed creating required GNSS-SDR NMEA PTY") from exc

    def _start_nmea_tty_reader(self) -> None:
        if not bool(self._cfg.gnss_pvt_nmea_tty_enable):
            return
        if self._nmea_master_fd is None or self._nmea_tty_path in {None, "/dev/null"}:
            raise RuntimeError("GNSS-SDR NMEA PTY was enabled but is unavailable")
        ...
        self._handoff_log.info(
            "GNSS-SDR NMEA tty monitor active: devname=%s rate_ms=%d",
            self._nmea_tty_path,
            int(self._cfg.gnss_pvt_nmea_rate_ms),
        )

    def _stop_nmea_tty_reader(self) -> bool:
        self._close_nmea_tty_fds()
    ...
    def _drain_stdout(self) -> None:
        ...
        try:
            if self._cfg.logging_enabled:
                ...
                console_handle = self._console_log_path.open(
                    "a", encoding="utf-8", errors="replace"
                )
                receiver_handle = self._receiver_log_path.open(
                    "a", encoding="utf-8", errors="replace"
                )
            line_buffer = ""
            while True:
            ...
            for handle in (console_handle, receiver_handle):
